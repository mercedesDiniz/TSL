...
%% Inverted Pendulumn linear model in continuous time domain
    ...
    C = [0 0 1 0]; ny = 1; % number of outputs
...
t = 0:Ts:N*Ts-Ts; % time vector for the plots
    ...
    r1(1:5)=0; r1(6:N)=0; % Null reference sequence

    % Disturbances at the output
      v = 0*wgn(1,N, 1e-5, 'linear'); % Noise
      v2(1:round(N/2))=0; v2(round(N/2)+1:N)=+0.2*0; % Load
...
%% Frequency response analysis in open-loop

    % LQR analysis: direct loop
    LQR_DL = ss(Aa, Ba, K, Da, Ts);
    figure; margin(LQR_DL);
