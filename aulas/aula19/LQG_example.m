...
Co = [Ba Aa*Ba (Aa^2)*Ba]; % Co = ctrb(Aa,Ba)
    ...
    %                    +Pcl(3)*Aa +Pcl(4)*Aa^0);
    Kacker = acker(Aa,Ba,roots(Pcl) );
        disp('MATLAB Ackermann K ='); disp(Kacker);
    
    % Solving using the LQR method
              % xa1 xa2  xa3
    Qlq = diag([ 1   50   1   ]);
    Rlq = 1;
    K = dlqr(Aa,Ba,Qlq,Rlq); disp('MATLAB LQR K ='); disp(K);
...
Ob = obsv(Aa,Ca)
    ...
    Lacker = (acker(Aa',Ca',roots(Pclo) ))';
    disp('MATLAB Ackermann L ='); disp(Lacker);
